...
export function createGrassField(scene, { meadows, corridor, stationPoints, fog }) {
  ...
  const maskTexture = makeMaskTexture(meadows, corridor, stationPoints);
  ...
  const uniforms = {
    ...
    uColorBase: { value: new THREE.Color('#5f7a25') },
    uColorTip: { value: new THREE.Color('#b8c23a') },
  ...
  const material = new THREE.ShaderMaterial({
    ...
    vertexShader: /* glsl */ `
      ...
      void main() {
        ...
        float half = uSize * 0.5;
        loop.x = mod(loop.x + half, uSize) - half;
        loop.y = mod(loop.y + half, uSize) - half;
  ...
  return {
    update(trainPos, camera, time) {
      uniforms.uCenter.value.set(trainPos.x, trainPos.z);
